...
. If a subset of to-be-renamed files have a file or directory in the
  ...
  user.
. If the other side of history did a directory rename to a path that
...
. Only apply implicit directory renames to directories if the other side
  of history is the one doing the renaming.
. Do not perform directory rename detection for directories which had no
