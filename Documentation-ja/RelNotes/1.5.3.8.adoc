 ...
 * git-svn fix to handle funky branch and project names going over http/https correctly.
 ...
 * git-send-email added redundant Message-Id: header to the outgoing e-mail when the patch text already had one.
 ...
 * After amending the patch title in "git-am -i", the command did not report the patch it applied with the updated title.
