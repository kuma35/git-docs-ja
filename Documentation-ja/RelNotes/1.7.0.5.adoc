 ...
 * "git stash show <commit-that-is-not-a-stash>" gave an error message that was not so useful. Reworded the message to "<it> is not a stash".
 ...
 * Smart http server backend failed to report an internal server error and infinitely looped instead after output pipe was closed.
