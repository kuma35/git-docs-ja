...
新しく作成されたリポジトリの最初のブランチには、 <branch-name> を使用します。指定しない場合は、デフォルトの名前にフォールバックします(現在は `master` ですが、Git 3.0 がリリースされると `main` に変更されます)。
endif::with-breaking-changes[]
ifdef::with-breaking-changes[]
新しく作成されたリポジトリの最初のブランチには、 <branch-name> を使用します。 指定しない場合は、デフォルトの名前 `main` にフォールバックします。
endif::with-breaking-changes[]
デフォルトの名前は、 `init.defaultBranch` 構成変数でカスタマイズできます。
