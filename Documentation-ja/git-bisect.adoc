...
「good」と「bad」の代わりに「old」と「new」を使用するには、 `git bisect start` を実行する時に、 引数にcommitを指定せずにおいて、 以下のコマンドを実行してコミットを指定する必要があります:

------------------------------------------------
git bisect old [<rev>]
------------------------------------------------

これは、このコミットが、 あなたの求める変更時点より前であったことを知らせるためで、あるいは、

------------------------------------------------
git bisect new [<rev>...]
------------------------------------------------

これは、 このコミットが、 あなたの求める変更時点より後であった事を知らせます。

現在使用している用語(terms)を忘れちゃったときは、以下を入力します。

------------------------------------------------
git bisect terms
------------------------------------------------

`git bisect terms --term-old` または `git bisect terms --term-good` は用語の old/good 用だけを出力するために使用できます。 `git bisect term --term-new` と `git bisect term --term-bad` は、 求めている変更よりも後のコミットを呼び出す方法を学ぶために使用できます(訳注:用語の new/bad 用だけを出力)

bad/good または new/old の代わりに独自の用語(terms)を使用する場合は、以下を使用してbisectを開始することにより、(`reset` 、`start` などの既存のbisectサブコマンドを除く)任意の名前を選択できます
...
次に、コミットをマークするために、 `git bisect good` や `git bisect bad` の代わりに、 `git bisect <term-old>` や `git bisect <term-new>` を使用します。

Bisect visualize/view
~~~~~~~~~~~~~~~~~~~~~

`gitk` 内で、 現在残っている容疑者(suspects)を確認するには、bisectプロセス中に以下のコマンドを発行します(サブコマンド `view` は `visualize` の代わりに使用できます):

------------
$ git bisect visualize
------------

Git は、 さまざまな環境変数を見てグラフィカル環境を検出します: `DISPLAY` は Unix システム上の X Window System 環境で設定されます。 `SESSIONNAME` は対話型デスクトップ・セッションの Cygwin で設定されます。 `MSYSTEM` は Msys2 および Windows 用 Git の下で設定されます。 `SECURITYSESSIONID` は macOS の対話型デスクトップ・セッションで設定されます。

これらの環境変数がいずれも設定されていない場合は、 代わりに `git log` が使用されます。 `-p` や `--stat` などのコマンドライン・オプションを指定することもできます。
...
これは、`v2.5` と `v2.6` (の両端を含む) の間のコミットをスキップする必要があることをbisectプロセスに通知します。
...
あなたが追跡している問題でツリーのどの部分が関係しているかがわかっている場合は、 `bisect start` コマンドを発行するときに pathspec パラメータを指定することで、試行回数をさらに減らすことができます:
...
`--no-checkout`::
+
bisect 処理の各反復で新しい作業ツリーをチェックアウトしないでください。 代わりに、 `BISECT_HEAD` という名前の参照を更新して、 テストする必要のあるコミットを指すようにします。
+
...
+
`test.sh` と `check_test_case.sh` の両方がリポジトリの外にある場合は、bisectプロセスとmakeプロセスとtestプロセスとスクリプトの間の相互作用を防ぐ方が安全です。
...
SEE ALSO
--------
link:git-bisect-lk2009.html[Fighting regressions with git bisect], linkgit:git-blame[1].
