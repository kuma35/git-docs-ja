...
ルーズ・オブジェクトをリクエストするバカ(dumb)クライアントの例:
...
クライアントは、サーバーに送信されるURLに空のパストークン(`//`)が表示されるのを防ぐために、ユーザーから与えられた $GIT_URL 文字列から、末尾に `/` がある場合はそれを削除しなければなりません。 互換性のあるクライアントは、`$GIT_URL/info/refs` を `foo//info/refs` ではなく、`foo/info/refs` として展開しなｋればなりません。
...
関連するヘッダーがリクエストに表れ、エンティティが変更されていない場合、サーバーは `304 Not Modified` を返す場合があります。 クライアントは、キャッシュされたエンティティを再利用することにより、 `304 Not Modified` を `200 OK` と同じように扱わなければいけません。
...
返されるコンテンツは、各参照とその既知の値を記述した UNIX 形式のテキストファイルです。 ファイルは、C ロケールの順序に従って名前でソートする必要があります。 ファイルには、 `HEAD` という名前のデフォルト参照を含めるべきではありません。
...
クライアントは、 追加パラメーター(Extra Parameters; linkgit:gitprotocol-pack[5] 参照) を、 Git-Protocol HTTP ヘッダーでコロンで区切られた文字列として送信できます。

`--http-backend-info-refs` オプションを linkgit:git-upload-pack[1] に使用します。
...
返される応答は、各参照とその既知の値を説明する pkt-line ストリームです。 ストリームは、C ロケールの順序に従って名前でソートする必要があります。 ストリームには、最初の参照として `HEAD` という名前のデフォルト参照を含める必要があります。 ストリームは、最初の参照の NUL の後ろに機能宣言(capability declarations)を含める必要があります。
...
クライアントは、最初に `$GIT_URL/info/refs?service=git-upload-pack` を使用して 参照 検出を実行しなければなりません。
...
クライアントは、リクエスト本文で少なくとも 1 つの `want` コマンドを送信する必要があります。 サーバーが機能 `allow-tip-sha1-in-want` または `allow-reachable-sha1-in-want` を宣伝(advertise)しない限り、クライアントは `want` コマンドで 参照 検出によって取得された応答に表示されなかった ID を参照してはいけません。
...
C: 広告された参照(advertised refs)を取得するために参照検出を使用します。
...
C: 参照発見中にクライアントが見た内容に基づいて、
   クライアントが取得したい `advertised` からのオブジェクトのセット `want` を構築する。
...
`have` リストは、 `c_pending` から最初の32のコミットを取り出して(pop)作成されます。 `c_pending` が空の場合、 それより少ない数が提供されることがあります。
...
`want` 内のすべてのオブジェクトが参照から直に到達可能であることを確認(verify)します。
...
各オブジェクトについて、サーバーに 参照 から到達可能なオブジェクトがある場合は、それを `s_common` に追加します。 コミットが s_common に追加された場合、祖先が have に含まれていても追加しないでください。
...
クライアントは、最初に `$GIT_URL/info/refs?service=git-receive-pack` を使用して 参照 検出を実行する必要があります。
...
クライアントは、リクエスト本文で少なくとも 1 つのコマンドを送信する必要があります。 リクエストボディのコマンド部分内で、クライアントは、参照 検出を通じて取得した ID を old_id として送信する必要があります。
...
https://www.ietf.org/rfc/rfc1738.txt[RFC 1738: Uniform Resource Locators (URL)] https://www.ietf.org/rfc/rfc2616.txt[RFC 2616: Hypertext Transfer Protocol -- HTTP/1.1]

SEE ALSO
--------

linkgit:gitprotocol-pack[5] linkgit:gitprotocol-capabilities[5]
