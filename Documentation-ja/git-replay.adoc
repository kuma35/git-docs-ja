...
NAME
----
git-replay - 実験的(EXPERIMENTAL): 新たなベース上でコミット群をリプレイします。 ベア・リポジトリでも動きます。
...
<revision-range>::
	リプレイするコミットの範囲。 複数の <revision-range> を渡すことができますが、 `--advance <branch>` モードでは、<branch> がどこを指すのかが明確になるように、 単一の先端(tip)であるべきです。 linkgit:git-rev-parse[1] の「Specifying Ranges」と、 下記の「Commit Limiting」オプションを参照してください。
