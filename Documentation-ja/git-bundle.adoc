...
NAME
----
git-bundle - オブジェクトやrefsを移動させるためにアーカイブします
...
リビジョンは、バンドルにパッケージ化される参照名を伴う必要があります。 あるいは、 `--all` を使用してすべての refs をパッケージ化することもできます。
...
ソース・リポジトリから直接クローンが取得するのと同一の refs セットを提供する場合は、 `<git-rev-list-args>` に `--branches --tags` を使用します。
...
まず、 リポジトリの完全バックアップについて考えてみましょう。 以下のコマンドは、 すべての refs がバンドルに含まれているという意味での、 リポジトリの完全バックアップを作成します:

----------------
$ git bundle create backup.bundle --all
----------------

ただし、 これは refs のみを対象としたものであることにもう一度注意してください。 つまり、 refs とそれらの refs から到達可能なコミットのみが含まれます。 その他のローカルの状態である、 インデックスの内容や、作業ツリーや、スタッシュや、リポジトリごとの設定や、フックなどは含めません。
...
このため、 リポジトリのバックアップを行うには Git ツールを使用することが推奨されています。 このコマンドや、 例えば linkgit:git-clone[1] などです。 しかし、 これらのツールは refs やコミット以外の状態をバックアップするのには役立ちません。 言い換えれば、 インデックスの内容や、 作業ツリーや、スタッシュや、リポジトリごとの設定や、フックなどのバックアップには役立ちません。
