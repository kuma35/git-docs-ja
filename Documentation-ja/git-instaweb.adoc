...
`-b`::
`--browser`::
	gitwebページを表示するために使用する必要があるWebブラウザー。これは、gitwebインスタンスのURLとともに `git web--browse` ヘルパースクリプトに渡されます。詳細については、 linkgit:git-web{litdd}browse[1] を参照してください。スクリプトが失敗した場合、URLはstdoutに出力されます。
...
構成変数 `instaweb.browser` が設定されていない場合に、`web.browser` が定義されていれば、代わりに `web.browser` が使用されます。詳細については、 linkgit:git-web{litdd}browse[1] を参照してください。
