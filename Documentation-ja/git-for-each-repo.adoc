...
NAME
----
git-for-each-repo - リポジトリのリストに対してGitコマンドを実行します
...
`--keep-going`::
	コマンドがとあるリポジトリで失敗した場合でも残りのリポジトリに対して続行します。 終了コードは、 引き続き、 操作全体が成功していないことを示します。
+
注意: 失敗したコマンドの終了コードは、 for-each-repo コマンドの終了コードになりません: 指定されたリポジトリのいずれかでコマンドが失敗した場合、 for-each-repo の終了コードは 1 になります。
...
各 `git -C <repo> <arguments>` サブプロセスは、標準のファイルデスクリプタ `stdin` と `stdout` と `stderr` を継承します。
